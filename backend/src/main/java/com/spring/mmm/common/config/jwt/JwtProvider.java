...
public class JwtProvider {
    ...
    private String createToken(String email, String nickname, Long tokenExpireTime) {
        Date date = new Date();
        return BEARER_PREFIX + Jwts.builder()
                .claim("email", email)//JWT의 주제(subject)를 사용자 이름으로 설정합니다.
     ...
     */
    public TokenResponse createTokenByLogin(String email, String nickname) {
        String accessToken = createToken(email, nickname, ACCESS_TOKEN_TIME);
        String refreshToken = createToken(email, nickname, REFRESH_TOKEN_TIME);
        redisDao.setRefreshToken(email, nickname, refreshToken, REFRESH_TOKEN_TIME);
        return new TokenResponse(accessToken, refreshToken);
    }

    //AccessToken 재발행 + refreshToken 함께 발행

    public TokenResponse reissueAtk(String email, String nickname, String reToken) {
    ...
    public Long getExpiration(String accessToken){
        //에세스 토큰 만료시간
        Date expiration = Jwts.parser().setSigningKey(key).build().parseClaimsJws(accessToken).getBody()
                .getExpiration();
        //현재시간
